    ...
    return (
        <section id="github" className="py-12 bg-transparent relative overflow-hidden">
            <div className="container mx-auto px-4 md:px-8 max-w-[1200px] relative z-10">
                    ...
                    <h2 className="text-3xl md:text-4xl font-black mb-3 tracking-tighter uppercase text-black">GitHub Activity</h2>
                    <div className="h-1.5 w-16 bg-black"></div>
                </div>

                <div className="grid grid-cols-2 gap-3 md:gap-4 mb-8">
                    {[
                        { label: 'Total Repositori', value: stats?.public_repos ?? '0', icon: BookOpen },
                        { label: 'Lihat Profil', value: `@${username}`, icon: Github, href: `https://github.com/${username}` }
                    ].map((stat, i) => {
                        const Wrapper = stat.href ? 'a' : 'div'
                        const wrapperProps = stat.href ? { href: stat.href, target: '_blank', rel: 'noreferrer' } : {}
                        return (
                            <Wrapper
                                key={i}
                                {...wrapperProps}
                ...
                <div className="bg-white border-2 border-black p-4 md:p-8 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 border-b-2 border-black pb-4">
                        <div className="flex items-center gap-3">
                            <Calendar size={20} />
                            <div>
                                <span className="text-sm font-black uppercase tracking-tight italic block">Grafik Kontribusi</span>
                                <span className="text-[10px] font-bold text-gray-500">Total Kontribusi {selectedYear}: {totalContributions}</span>
                            </div>
                        </div>
                    ...
                    <div className="flex justify-start md:justify-center min-h-[160px] items-center overflow-x-auto pb-6 custom-scrollbar">
                        {loading ? (
                            <div className="flex flex-col items-center gap-2 text-gray-400 italic mx-auto">
                                <Loader2 className="h-8 w-8 animate-spin" />
                                <span className="text-xs font-black">Sync_Data</span>
                            </div>
                        ) : contributions.length > 0 ? (
                            <div className="relative p-4 border border-black/5 bg-[#fcfcfc] rounded-sm">
                                <div className="flex gap-[2px] md:gap-[4px]">
                                    {weeks.map((week, weekIndex) => (
                                        <div key={weekIndex} className="flex flex-col gap-[2px] md:gap-[4px]">
                                            {week.map((day, dayIndex) => (
                                                <div
                                                    key={day.date || `empty-${weekIndex}-${dayIndex}`}
                                                    className={`w-[8px] h-[8px] md:w-[12px] md:h-[12px] border transition-all relative ${day.date ? 'hover:border-black cursor-pointer group' : ''}`}
                                                    style={{
                                                ...
                                                >
                                                    {day.date && (
                                                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-black text-white text-[8px] font-bold whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none z-20 rounded shadow-lg transition-opacity">
                                                            <span className="text-green-400">{day.count}</span> kontribusi pada {new Date(day.date).toLocaleDateString('id-ID', { month: 'short', day: 'numeric', year: 'numeric' })}
                                                        </div>
                                                    )}
                                                </div>
                                            ))}
                            ...
                            <div className="flex flex-col items-center gap-2 text-gray-400 italic mx-auto">
                                ...
                                <span className="text-xs font-black uppercase">BELUM_SETUP</span>
                                <p className="text-[10px] font-medium max-w-xs text-center">
                                    Tambahkan GitHub token pada file .env.local
                                </p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="mt-6 pt-4 border-t border-black border-dashed flex flex-wrap justify-between items-center gap-4 text-[10px] font-black italic text-gray-400">
                        ...
                        <div className="flex items-center gap-2">
                            <span className="text-black uppercase text-[8px]">Sedikit</span>
                            {[0, 1, 2, 3, 4].map(level => (
                                <div key={level} className="w-3 h-3 border border-gray-300" style={{ backgroundColor: getColor(level) }} />
                            ))}
                            <span className="text-black uppercase text-[8px]">Banyak</span>
                        </div>
                    </div>
                </div>
            </div>
